...
export async function addToCart(listingId, quantity = 1) {
  ...
  const { data: listing, error: listingError } = await supabase.from('listings').select('id').eq('id', listingId).maybeSingle();
  ...
  if (existing) {
    const { error } = await supabase.from('cart_items').update({ quantity: Number(existing.quantity) + safeQuantity }).eq('id', existing.id).eq('user_id', user.id);
    return error ? { ok: false, error } : { ok: true };
  }
  const { error } = await supabase.from('cart_items').insert({ user_id: user.id, listing_id: listingId, quantity: safeQuantity });
  return error ? { ok: false, error } : { ok: true };
}
...
export async function updateCartItem(itemId, quantity) {
  ...
  const { error } = await supabase.from('cart_items').update({ quantity: safeQuantity }).eq('id', itemId).eq('user_id', user.id);
  return error ? { ok: false, error } : { ok: true };
}
