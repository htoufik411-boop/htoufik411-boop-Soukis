...
export function installProductsUI(){
 ...
 function fillFilters(preserve){
  ...
  const cats=document.querySelector('#cats');if(cats)cats.innerHTML=categories.slice(0,12).map(c=>`<button class="cat" data-cat="${esc(c.id)}"><i>▦</i>${esc(label(c))}</button>`).join('');
...
function card(p){const id=esc(p.id),title=esc(p.title||p.name||text('منتج','Produit','Product')),cat=esc(categoryLabel(p)||text('عام','Général','General')),location=esc(cityLabel(p)),price=Number(p.price||0).toLocaleString(locale()),image=esc(p.image_url||p.image||'');return `<article class="product" data-id="${id}"><div class="pic">${image?`<img src="${image}" alt="${title}" loading="lazy">`:'<span class="emoji">🛍️</span>'}</div><div class="body"><small>${cat}</small><h3>${title}</h3><div class="price">${price} ${currencyLabel()}</div><div class="loc">${location}</div><button type="button" class="cart" data-add-listing="${id}" data-listing-id="${id}">🛒 ${text('أضف إلى السلة','Ajouter au panier','Add to cart')}</button></div></article>`;}
