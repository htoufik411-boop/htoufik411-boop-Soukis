...
export function installI18n() {
  ...
  const apply = lang => {
    ...
    const search = document.querySelector('#search'); if (search) search.placeholder = t.search;
    const category = document.querySelector('#category'); if (category && !category.value) category.options[0].textContent = t.all;
    const sort = document.querySelector('#sort'); if (sort && sort.options[0]) sort.options[0].textContent = t.latest;
  ...
  const saved = localStorage.getItem('soukis-language') || 'ar';
