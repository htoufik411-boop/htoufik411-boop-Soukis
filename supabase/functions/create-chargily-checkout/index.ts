...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey || !secret || !successUrl || !failureUrl || !webhookEndpoint) {
  ...
  let checkout: any;
  try {
    checkout = JSON.parse(providerBody);
  } catch {
    checkout = null;
  }
  ...
  const { data: bound, error: bindError } = await serviceClient.rpc("bind_chargily_checkout", {
