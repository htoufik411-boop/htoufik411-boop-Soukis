...
function metadataObject(metadata: unknown): Record<string, unknown> {
  ...
  if (Array.isArray(metadata) && metadata[0] && typeof metadata[0] === "object") {
    return metadata[0] as Record<string, unknown>;
  }
  return {};
}
...
Deno.serve(async (req) => {
  ...
  // Only successful checkout events can move a corporate ad payment to paid.
  if (event?.type !== "checkout.paid") return json({ received: true });

  const checkout = event?.data;
  if (!checkout || checkout.status !== "paid") {
    return json({ error: "invalid_paid_event" }, 400);
  }
  ...
  const currency = typeof checkout.currency === "string" ? checkout.currency : "DZD";
  const metadata = metadataObject(checkout.metadata);
  const requestId = typeof metadata.request_id === "string" ? metadata.request_id : "";

  if (!eventId || !checkoutId || !requestId || !Number.isFinite(amount)) {
    return json({ error: "missing_payment_fields" }, 400);
  }
