...
create table if not exists public.corporate_ad_campaigns (
  id uuid primary key default gen_random_uuid(),
  request_id uuid not null unique references public.corporate_ad_requests(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  placement text not null check (placement in ('listing_feed','homepage','top_banner','multi_placement')),
...
create table if not exists public.corporate_ad_payments (
  id uuid primary key default gen_random_uuid(),
  request_id uuid not null references public.corporate_ad_requests(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  amount_dzd numeric(12,2) not null check (amount_dzd > 0),
...
create table if not exists public.corporate_ad_plans (
  ...
  duration_days integer not null check (duration_days in (7,15,21,30)),
  base_amount_dzd numeric(12,2) not null check (base_amount_dzd > 0),
  active boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

insert into public.corporate_ad_plans(plan,duration_days,base_amount_dzd)
values ('starter',7,5000),('business',15,9000),('premium',21,12000),('enterprise',30,15000)
...
revoke all on public.corporate_ad_requests from anon, authenticated;
grant select on public.corporate_ad_requests to authenticated;
revoke all on public.corporate_ad_campaigns from anon, authenticated;
grant select on public.corporate_ad_campaigns to authenticated;
revoke all on public.corporate_ad_payments from anon, authenticated;
grant select on public.corporate_ad_payments to authenticated;
revoke all on public.corporate_ad_plans from anon, authenticated;
grant select on public.corporate_ad_plans to authenticated;

create or replace function public.create_corporate_ad_request(
  p_company_name text, p_contact_name text, p_contact_email text, p_contact_phone text,
  p_ad_title text, p_ad_description text, p_destination_url text,
  p_placement text, p_plan text, p_creative_url text
) returns public.corporate_ad_requests
language plpgsql security definer set search_path = public
as $$
declare
  v_user uuid := auth.uid();
  v_plan public.corporate_ad_plans%rowtype;
  v_multiplier numeric := case p_placement when 'homepage' then 2 when 'top_banner' then 1.5 when 'multi_placement' then 2.5 else 1 end;
...
begin
  ...
  select * into v_plan from public.corporate_ad_plans where plan=p_plan and active=true;
  ...
  values (v_user,trim(p_company_name),trim(p_contact_name),lower(trim(p_contact_email)),nullif(trim(p_contact_phone),''),trim(p_ad_title),nullif(trim(p_ad_description),''),nullif(trim(p_destination_url),''),p_placement,p_plan,v_plan.duration_days,v_plan.base_amount_dzd*v_multiplier,nullif(trim(p_creative_url),''))
  returning * into v_row;
  return v_row;
end;
$$;

revoke all on function public.create_corporate_ad_request(text,text,text,text,text,text,text,text,text,text) from public, anon;
grant execute on function public.create_corporate_ad_request(text,text,text,text,text,text,text,text,text,text) to authenticated;
