-- Soukis: safe Chargily integration
-- Built from the reviewed logic in PRs #15/#16, with strict RPC privileges.
...
begin
  ...
  return jsonb_build_object(
    ...
    'payment_method',case when v_payment.payment_method in ('edahabia','cib','chargily_app') then v_payment.payment_method else null end,
...
begin
  ...
  if upper(trim(coalesce(p_currency,''))) <> 'DZD' then raise exception 'invalid_currency'; end if;
  ...
  from public.corporate_ad_payments
  where provider_checkout_id = trim(p_checkout_id)
  for update;

  if v_event_payment.id is not null
     and v_checkout_payment.id is not null
     and v_event_payment.id is distinct from v_checkout_payment.id then
  ...
  if v_payment.provider_checkout_id <> trim(p_checkout_id) then raise exception 'checkout_id_mismatch'; end if;
  if v_payment.request_id <> p_request_id then raise exception 'request_mismatch'; end if;
  if upper(v_payment.currency) <> upper(trim(p_currency)) then raise exception 'currency_mismatch'; end if;
