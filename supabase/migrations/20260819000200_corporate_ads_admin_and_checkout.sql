...
begin
  ...
  return jsonb_build_object(
    ...
    'amount_dzd', v_request.amount_dzd,
    'reviewed_at', v_request.reviewed_at
  );
end;
$$;

revoke all on function public.admin_review_corporate_ad(uuid,text,text) from public, anon, authenticated;
grant execute on function public.admin_review_corporate_ad(uuid,text,text) to authenticated;
...
begin
  ...
  values (
    v_request.id, v_user, v_request.amount_dzd, p_payment_provider, p_payment_method
  )
