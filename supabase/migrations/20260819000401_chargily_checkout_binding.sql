...
begin
  ...
  if v_payment.provider_checkout_id is not null then
    return jsonb_build_object('status','already_created','payment_id',v_payment.id,'checkout_id',v_payment.provider_checkout_id);
  end if;
  ...
  set checkout_creation_token = v_token, checkout_creation_started_at = now()
  where id = v_payment.id;

  v_method := case when v_payment.payment_method in ('edahabia','cib','chargily_app') then v_payment.payment_method else null end;

  return jsonb_build_object(
    'status','ready','payment_id',v_payment.id,'request_id',v_request.id,
    'amount_dzd',v_payment.amount_dzd,'currency',lower(v_payment.currency),
    'payment_method',v_method,'creation_token',v_token
  );
end;
$$;
...
create or replace function public.bind_chargily_checkout(
  p_payment_id uuid, p_creation_token uuid, p_checkout_id text
) returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.corporate_ad_payments
  set provider_checkout_id = trim(p_checkout_id), checkout_creation_token = null, checkout_creation_started_at = null
  where id = p_payment_id and user_id = v_user and payment_provider = 'chargily'
    and status = 'pending' and checkout_creation_token = p_creation_token
  returning * into v_payment;
  if not found then raise exception 'checkout_binding_failed'; end if;

  return jsonb_build_object('status','bound','payment_id',v_payment.id,'checkout_id',v_payment.provider_checkout_id);
...
-- Strict provider correlation: request_id alone can never activate a payment.
create or replace function public.process_chargily_checkout_paid(
  p_event_id text, p_checkout_id text, p_request_id uuid,
  p_amount numeric, p_currency text, p_payment_method text default null
) returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select * into v_payment
  from public.corporate_ad_payments
  where provider_event_id = p_event_id or provider_checkout_id = p_checkout_id
  ...
  update public.corporate_ad_payments
  set status='paid', paid_at=now(), provider_transaction_id=coalesce(provider_transaction_id, trim(p_checkout_id)), provider_event_id=p_event_id, payment_method=coalesce(p_payment_method, payment_method)
  where id=v_payment.id returning * into v_payment;

  update public.corporate_ad_requests set status='paid', updated_at=now() where id=v_request.id;

  insert into public.corporate_ad_campaigns(request_id,user_id,placement,starts_at,ends_at,status)
  values(v_request.id,v_request.user_id,v_request.placement,now(),now()+make_interval(days=>v_request.duration_days),'active')
  on conflict(request_id) do update set starts_at=excluded.starts_at,ends_at=excluded.ends_at,status='active',updated_at=now()
  returning * into v_campaign;

  update public.corporate_ad_requests set status='active',updated_at=now() where id=v_request.id;
  return jsonb_build_object('status','active','payment_id',v_payment.id,'request_id',v_request.id,'campaign_id',v_campaign.id);
end;
$$;
