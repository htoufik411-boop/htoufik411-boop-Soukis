-- Corporate Ads: admin workflow helpers.
-- Kept at a later migration timestamp so it runs after the admin/checkout migration.
...
begin
  ...
  return query
  select r.*
  from public.corporate_ad_requests r
...
-- The previous migration creates this RPC with a jsonb return type. PostgreSQL
-- cannot change a function return type with CREATE OR REPLACE, so remove the
-- previous signature before installing the campaign-returning admin workflow.
...
begin
  ...
  if v_payment.status = 'paid' then
    ...
    from public.corporate_ad_campaigns
  ...
  values (
    ...
    v_request.placement,
    v_now,
    v_now + make_interval(days => v_request.duration_days),
    'active'
  )
