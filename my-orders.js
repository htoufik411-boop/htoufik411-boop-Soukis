...
export async function getMyOrders(){const {data:{user}}=await supabase.auth.getUser();if(!user)return{ok:false,reason:'auth_required',orders:[]};const{data,error}=await supabase.from('orders').select('id,status,total,currency,shipping_name,shipping_phone,shipping_address,created_at,order_items(id,listing_id,title,quantity,unit_price,listings(title,name))').eq('buyer_id',user.id).order('created_at',{ascending:false});return error?{ok:false,error,orders:[]}:{ok:true,orders:data||[]};}
